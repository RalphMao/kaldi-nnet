...
class ConvolutionalComponent : public UpdatableComponent {
 public:
  ...
  void ReadData(std::istream &is, bool binary) {
    ...
    ExpectToken(is,binary, "<FilterSection>");
    ReadBasicType(is,binary, &filter_section_);
    ExpectToken(is,binary, "<FilterStep>");
    ReadBasicType(is,binary, &filter_step_);
    // re-scale learn rate
    ...
    KALDI_ASSERT((patch_stride_ - filter_section_) % filter_step_ == 0);
    int32 num_sections = 1 + (patch_stride_ - filter_section_)/ filter_step_;

    //number of patches per section:

    KALDI_ASSERT((filter_section_ - patch_dim_) % patch_step_ == 0);
    int32 num_patches = 1 + (filter_section_ - patch_dim_) / patch_step_;
    ...
    KALDI_ASSERT(output_dim_ % (num_patches*num_sections) == 0);
    int32 num_filters = output_dim_ * num_sections / num_patches / num_sections;
...
//    //
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    int32 num_sections = 1 + (patch_stride_ - filter_section_)/ filter_step_;
    int32 num_patches = 1 + (filter_section_ - patch_dim_) / patch_step_;
    int32 num_filters_sec = num_filters/num_sections;
	...
	for (int32 p=0; p<num_patches;p++)
	{
	    ...
	    CuSubMatrix<BaseFloat> tgt(out->ColRange((s * num_patches+p) * num_filters_sec, num_filters_sec));
	    tgt.AddVecToRows(1.0, bias_.Range(s*num_filters_sec,num_filters_sec), 0.0); // add bias
	    tgt.AddMatMat(1.0,in.ColRange((s*filter_step_+p*patch_step_)*num_splice,filter_dim), kNoTrans, 
		    filters_.RowRange(s*num_filters_sec,num_filters_sec), kTrans, 1.0);
	}
    }
...
*/
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    CuMatrix<BaseFloat> feature_patch_diff;
    feature_patch_diff.Resize(num_frames,filter_dim,kSetZero);
    for (int32 s=0; s<num_sections; s++) {
    for (int32 p=0; p<num_patches; p++) {
...
*/
  ...
  void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    filters_grad_.Resize(num_filters_sec, filter_dim, kSetZero); // reset
    bias_grad_.Resize(num_filters_sec, kSetZero); // reset

    
    for (int32 p=0; p<num_patches; p++) { // sum
      CuSubMatrix<BaseFloat> diff_patch(diff.ColRange((s * num_patches+p) * num_filters_sec, num_filters_sec));
      filters_grad_.AddMatMat(1.0, diff_patch, kTrans, input.ColRange((s*filter_step_+p*patch_step_)*num_splice,filter_dim), kNoTrans, 1.0);
...
*/    //
 ...
 private:
  int32 filter_section_,
	filter_step_,
	patch_dim_,    ///< number of consecutive inputs, 1st dim of patch
