 ...
 public:
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in, CuMatrixBase<BaseFloat> *out) {
    ...
    for (int32 p=0; p<num_patches;p++) {
        ...
        // apply all filters
        tgt.AddMatMat(1.0,in.ColRange(p*patch_step_*num_splice,filter_dim), kNoTrans, filters_, kTrans, 1.0);
		}
...
*/
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in, const CuMatrixBase<BaseFloat> &out,
    ...
    for (int32 p=0; p<num_patches; p++) {
      ...
      feature_patch_diff.AddMatMat(1.0, out_diff_patch, kNoTrans, filters_, kNoTrans, 0.0);
      CuSubMatrix<BaseFloat> tgt(in_diff->ColRange(p*patch_step_*num_splice,filter_dim));
...
*/
  ...
  void Update(const CuMatrixBase<BaseFloat> &input, const CuMatrixBase<BaseFloat> &diff) {
    ...
    for (int32 p=0; p<num_patches; p++) { // sum
      ...
      filters_grad_.AddMatMat(1.0, diff_patch, kTrans, input.ColRange(p*patch_step_*num_splice,filter_dim), kNoTrans, 1.0);
